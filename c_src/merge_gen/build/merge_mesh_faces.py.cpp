...
const char* const merge_mesh_faces_kwlist[] = {
  "I_depth",
  "P_faces",
  "I_Vidx",
  "out_verts",
...
// merge_mesh_faces
PyObject *merge_mesh_faces(PyObject *module, PyObject *args, PyObject *kwargs) {
  PyObject* py_I_depth;
  PyObject* py_P_faces;
  PyObject* py_I_Vidx;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOOOO", (char**)merge_mesh_faces_kwlist
    , &py_I_depth
    , &py_P_faces
    , &py_I_Vidx
    , &py_out_verts
  ...
  PyHalideBuffer<2> b_I_Vidx;
  PyHalideBuffer<2> b_out_verts;
  PyHalideBuffer<2> b_out_uvs;
  PyHalideBuffer<2> b_out_faces;
  if (!b_I_depth.unpack(py_I_depth, 0, merge_mesh_faces_kwlist[0])) return nullptr;
  if (!b_P_faces.unpack(py_P_faces, 0, merge_mesh_faces_kwlist[1])) return nullptr;
  if (!b_I_Vidx.unpack(py_I_Vidx, 0, merge_mesh_faces_kwlist[2])) return nullptr;
  if (!b_out_verts.unpack(py_out_verts, PyBUF_WRITABLE, merge_mesh_faces_kwlist[3])) return nullptr;
  if (!b_out_uvs.unpack(py_out_uvs, PyBUF_WRITABLE, merge_mesh_faces_kwlist[4])) return nullptr;
  if (!b_out_faces.unpack(py_out_faces, PyBUF_WRITABLE, merge_mesh_faces_kwlist[5])) return nullptr;

  b_I_depth.halide_buf.set_host_dirty();
  b_P_faces.halide_buf.set_host_dirty();
  b_I_Vidx.halide_buf.set_host_dirty();
  int result;
  Py_BEGIN_ALLOW_THREADS
  result = merge_mesh_faces(
...
#define HALIDE_PYTHON_EXTENSION_MODULE_NAME merge_mesh_faces
#endif  // HALIDE_PYTHON_EXTENSION_MODULE_NAME

#ifndef HALIDE_PYTHON_EXTENSION_FUNCTIONS
#define HALIDE_PYTHON_EXTENSION_FUNCTIONS X(merge_mesh_faces)
