...
const char* const merge_mesh_verts_kwlist[] = {
  "I_fore_idx",
  "I_back_idx",
  "I_back_labels",
  "BI_fore_mask",
...
// merge_mesh_verts
PyObject *merge_mesh_verts(PyObject *module, PyObject *args, PyObject *kwargs) {
  PyObject* py_I_fore_idx;
  PyObject* py_I_back_idx;
  PyObject* py_I_back_labels;
  PyObject* py_BI_fore_mask;
  int py_N0_verts;
  PyObject* py_P_out_faces;
  PyObject* py_P_N_faces;
  PyObject* py_I_out_vidx;
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOOiOOO", (char**)merge_mesh_verts_kwlist
    , &py_I_fore_idx
    , &py_I_back_idx
    , &py_I_back_labels
    , &py_BI_fore_mask
  ...
  PyHalideBuffer<2> b_I_fore_idx;
  PyHalideBuffer<2> b_I_back_idx;
  PyHalideBuffer<2> b_I_back_labels;
  PyHalideBuffer<2> b_BI_fore_mask;
  PyHalideBuffer<3> b_P_out_faces;
  PyHalideBuffer<1> b_P_N_faces;
  PyHalideBuffer<2> b_I_out_vidx;
  if (!b_I_fore_idx.unpack(py_I_fore_idx, 0, merge_mesh_verts_kwlist[0])) return nullptr;
  if (!b_I_back_idx.unpack(py_I_back_idx, 0, merge_mesh_verts_kwlist[1])) return nullptr;
  if (!b_I_back_labels.unpack(py_I_back_labels, 0, merge_mesh_verts_kwlist[2])) return nullptr;
  if (!b_BI_fore_mask.unpack(py_BI_fore_mask, 0, merge_mesh_verts_kwlist[3])) return nullptr;
  if (!b_P_out_faces.unpack(py_P_out_faces, PyBUF_WRITABLE, merge_mesh_verts_kwlist[5])) return nullptr;
  if (!b_P_N_faces.unpack(py_P_N_faces, PyBUF_WRITABLE, merge_mesh_verts_kwlist[6])) return nullptr;
  if (!b_I_out_vidx.unpack(py_I_out_vidx, PyBUF_WRITABLE, merge_mesh_verts_kwlist[7])) return nullptr;

  b_I_fore_idx.halide_buf.set_host_dirty();
  b_I_back_idx.halide_buf.set_host_dirty();
  b_I_back_labels.halide_buf.set_host_dirty();
  b_BI_fore_mask.halide_buf.set_host_dirty();
  int result;
  Py_BEGIN_ALLOW_THREADS
  result = merge_mesh_verts(
    &b_I_fore_idx.halide_buf,
    &b_I_back_idx.halide_buf,
    &b_I_back_labels.halide_buf,
    &b_BI_fore_mask.halide_buf,
    py_N0_verts,
    &b_P_out_faces.halide_buf,
    &b_P_N_faces.halide_buf,
    &b_I_out_vidx.halide_buf
  );
  Py_END_ALLOW_THREADS
  if (result == 0) result = halide_copy_to_host(nullptr, &b_P_out_faces.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_P_N_faces.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_I_out_vidx.halide_buf);
  if (result != 0) {
...
#define HALIDE_PYTHON_EXTENSION_MODULE_NAME merge_mesh_verts
#endif  // HALIDE_PYTHON_EXTENSION_MODULE_NAME

#ifndef HALIDE_PYTHON_EXTENSION_FUNCTIONS
#define HALIDE_PYTHON_EXTENSION_FUNCTIONS X(merge_mesh_verts)
