...
const char* const foreground_mesh_faces_kwlist[] = {
  "I_depth",
  "P_faces",
  "I_Vlabels",
  "Vidx_init",
...
PyObject *foreground_mesh_faces(PyObject *module, PyObject *args, PyObject *kwargs) {
  PyObject* py_I_depth;
  PyObject* py_P_faces;
  PyObject* py_I_Vlabels;
  int py_Vidx_init;
  PyObject* py_out_v_idx;
  PyObject* py_out_verts;
  PyObject* py_out_uvs;
  PyObject* py_out_faces;
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOiOOOO", (char**)foreground_mesh_faces_kwlist
    , &py_I_depth
    , &py_P_faces
    , &py_I_Vlabels
    , &py_Vidx_init
  ...
  PyHalideBuffer<2> b_I_depth;
  PyHalideBuffer<3> b_P_faces;
  PyHalideBuffer<2> b_I_Vlabels;
  PyHalideBuffer<2> b_out_v_idx;
  PyHalideBuffer<2> b_out_verts;
  PyHalideBuffer<2> b_out_uvs;
  PyHalideBuffer<2> b_out_faces;
  if (!b_I_depth.unpack(py_I_depth, 0, foreground_mesh_faces_kwlist[0])) return nullptr;
  if (!b_P_faces.unpack(py_P_faces, 0, foreground_mesh_faces_kwlist[1])) return nullptr;
  if (!b_I_Vlabels.unpack(py_I_Vlabels, 0, foreground_mesh_faces_kwlist[2])) return nullptr;
  if (!b_out_v_idx.unpack(py_out_v_idx, PyBUF_WRITABLE, foreground_mesh_faces_kwlist[4])) return nullptr;
  if (!b_out_verts.unpack(py_out_verts, PyBUF_WRITABLE, foreground_mesh_faces_kwlist[5])) return nullptr;
  if (!b_out_uvs.unpack(py_out_uvs, PyBUF_WRITABLE, foreground_mesh_faces_kwlist[6])) return nullptr;
  if (!b_out_faces.unpack(py_out_faces, PyBUF_WRITABLE, foreground_mesh_faces_kwlist[7])) return nullptr;

  b_I_depth.halide_buf.set_host_dirty();
  b_P_faces.halide_buf.set_host_dirty();
  b_I_Vlabels.halide_buf.set_host_dirty();
  int result;
  Py_BEGIN_ALLOW_THREADS
  result = foreground_mesh_faces(
    &b_I_depth.halide_buf,
    &b_P_faces.halide_buf,
    &b_I_Vlabels.halide_buf,
    py_Vidx_init,
    &b_out_v_idx.halide_buf,
    &b_out_verts.halide_buf,
    &b_out_uvs.halide_buf,
    &b_out_faces.halide_buf
  );
  Py_END_ALLOW_THREADS
  if (result == 0) result = halide_copy_to_host(nullptr, &b_out_v_idx.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_out_verts.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_out_uvs.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_out_faces.halide_buf);
  if (result != 0) {
...
#define HALIDE_PYTHON_EXTENSION_MODULE_NAME foreground_mesh_faces
#endif  // HALIDE_PYTHON_EXTENSION_MODULE_NAME

#ifndef HALIDE_PYTHON_EXTENSION_FUNCTIONS
#define HALIDE_PYTHON_EXTENSION_FUNCTIONS X(foreground_mesh_faces)
