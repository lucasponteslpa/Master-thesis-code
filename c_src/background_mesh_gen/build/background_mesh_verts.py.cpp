...
int background_mesh_verts(struct halide_buffer_t *_canny_buffer, struct halide_buffer_t *_back_canny_buffer, struct halide_buffer_t *_depth_buffer, int32_t _block_stride_x, int32_t _block_stride_y, struct halide_buffer_t *_BI_merge_mask_buffer, struct halide_buffer_t *_I_Vcoords_buffer, struct halide_buffer_t *_I_UVcoords_buffer, struct halide_buffer_t *_I_labels_buffer);

HALIDE_FUNCTION_ATTRS
int background_mesh_verts_argv(void **args);

HALIDE_FUNCTION_ATTRS
const struct halide_filter_metadata_t *background_mesh_verts_metadata();
...
const char* const background_mesh_verts_kwlist[] = {
  "canny",
  "back_canny",
  "depth",
  "block_stride_x",
...
PyObject *background_mesh_verts(PyObject *module, PyObject *args, PyObject *kwargs) {
  PyObject* py_canny;
  PyObject* py_back_canny;
  PyObject* py_depth;
  int py_block_stride_x;
  int py_block_stride_y;
  PyObject* py_BI_merge_mask;
  PyObject* py_I_Vcoords;
  PyObject* py_I_UVcoords;
  PyObject* py_I_labels;
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOiiOOOO", (char**)background_mesh_verts_kwlist
    , &py_canny
    , &py_back_canny
    , &py_depth
    , &py_block_stride_x
  ...
  PyHalideBuffer<2> b_canny;
  PyHalideBuffer<2> b_back_canny;
  PyHalideBuffer<2> b_depth;
  PyHalideBuffer<2> b_BI_merge_mask;
  PyHalideBuffer<3> b_I_Vcoords;
  PyHalideBuffer<3> b_I_UVcoords;
  PyHalideBuffer<2> b_I_labels;
  if (!b_canny.unpack(py_canny, 0, background_mesh_verts_kwlist[0])) return nullptr;
  if (!b_back_canny.unpack(py_back_canny, 0, background_mesh_verts_kwlist[1])) return nullptr;
  if (!b_depth.unpack(py_depth, 0, background_mesh_verts_kwlist[2])) return nullptr;
  if (!b_BI_merge_mask.unpack(py_BI_merge_mask, PyBUF_WRITABLE, background_mesh_verts_kwlist[5])) return nullptr;
  if (!b_I_Vcoords.unpack(py_I_Vcoords, PyBUF_WRITABLE, background_mesh_verts_kwlist[6])) return nullptr;
  if (!b_I_UVcoords.unpack(py_I_UVcoords, PyBUF_WRITABLE, background_mesh_verts_kwlist[7])) return nullptr;
  if (!b_I_labels.unpack(py_I_labels, PyBUF_WRITABLE, background_mesh_verts_kwlist[8])) return nullptr;

  b_canny.halide_buf.set_host_dirty();
  b_back_canny.halide_buf.set_host_dirty();
  b_depth.halide_buf.set_host_dirty();
  int result;
  Py_BEGIN_ALLOW_THREADS
  result = background_mesh_verts(
    &b_canny.halide_buf,
    &b_back_canny.halide_buf,
    &b_depth.halide_buf,
    py_block_stride_x,
    py_block_stride_y,
    &b_BI_merge_mask.halide_buf,
    &b_I_Vcoords.halide_buf,
    &b_I_UVcoords.halide_buf,
    &b_I_labels.halide_buf
  );
  Py_END_ALLOW_THREADS
  if (result == 0) result = halide_copy_to_host(nullptr, &b_BI_merge_mask.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_I_Vcoords.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_I_UVcoords.halide_buf);
  if (result == 0) result = halide_copy_to_host(nullptr, &b_I_labels.halide_buf);
  if (result != 0) {
...
#define HALIDE_PYTHON_EXTENSION_MODULE_NAME background_mesh_verts
#endif  // HALIDE_PYTHON_EXTENSION_MODULE_NAME

#ifndef HALIDE_PYTHON_EXTENSION_FUNCTIONS
#define HALIDE_PYTHON_EXTENSION_FUNCTIONS X(background_mesh_verts)
