...
class DualPixelsTextureRenderer:
    ...
    def getProgramLocations(self):
        ...
        self.Mmodel_id = glGetUniformLocation(self.program_id, "model_matrix")
        self.Mview_id = glGetUniformLocation(self.program_id, "view_matrix")
        self.dcoeffs_id = glGetUniformLocation(self.program_id, "dist_coeffs")
        self.ppoints_id = glGetUniformLocation(self.program_id, "principal_points")
        self.Mproj_id = glGetUniformLocation(self.program_id, "projection_matrix")
    ...
    def runMVP(self, intrinsics, coeffs, principal_p, angles, center, get_screenshot=False):
        ...
        # for i, mvp in enumerate(view_list):
            ...
            self.getPerspectiveMatrix(intrinsics=intrinsics)
            c2w = np.eye(4)
            c2w[:3,:3] = cross_product_matrix_function(angles)
            c2w[:3,-1] = center
            # breakpoint()
            self.getViewMatrix(c2w=c2w)
            ...
            glUniformMatrix4fv(self.Mmodel_id, 1, GL_FALSE, self.M_mat.data)
            glUniformMatrix4fv(self.Mview_id, 1, GL_FALSE, self.V_mat.data)
            glUniformMatrix4fv(self.Mproj_id, 1, GL_FALSE, self.P_mat.data)
            glUniform1f(self.dcoeffs_id, 3, GL_FALSE, (ctypes.c_float * len(coeffs))(*coeffs.tolist()))
            glUniform1f(self.ppoints_id, 4, GL_FALSE, (ctypes.c_float * len(principal_p))(*principal_p.tolist()))
